...
public class Tweaks
{
    ...
    [Range(0, 1)] public float TransparencyValue = 0.1f;
}
...
public class Valve
{
    ...
    [HideInInspector] public Vector3 DefPos;
}
...
{
    [Header("System")] public EngineElements elements;
    ...
    [Header("Contorls")] public bool ShowGUI;

    [Range(0, 30)] public float RPM = 10;
    ...
    private void Start()
    {
        ...
        foreach (Valve valve in elements.intakeValves)
        {
            valve.DefPos = valve.ValveGameobject.transform.localPosition;
        }

        foreach (Valve valve in elements.exhaustValves)
        {
            valve.DefPos = valve.ValveGameobject.transform.localPosition;
        }
    }
    ...
    private void OnGUI()
    {
        ...
        if (GUI.Button(new Rect(Screen.width - 200, 30, 95, 20), "Tuning 1"))
        {
            SetVariation(0);
        }

        if (GUI.Button(new Rect(Screen.width - 100, 30, 95, 20), "Tuning 2"))
        {
            SetVariation(1);
        }

        if (GUI.Button(new Rect(Screen.width - 200, 55, 95, 20), "Tuning 3"))
        {
            SetVariation(2);
        }

        if (GUI.Button(new Rect(Screen.width - 100, 55, 95, 20), "Tuning 4"))
        {
            SetVariation(3);
        }

        if (GUI.Button(new Rect(Screen.width - 200, 80, 195, 20), "Enable transparency"))
        {
            EnableTransparency();
        }

        if (GUI.Button(new Rect(Screen.width - 200, 105, 195, 20), "Disable transparency"))
        {
            DisableTransparency();
        }

        elements.EngineBlock.SetActive(GUI.Toggle(new Rect(Screen.width - 200, 130, 200, 20),
            elements.EngineBlock.activeSelf, "Engine block"));
        elements.CylinderHead.SetActive(GUI.Toggle(new Rect(Screen.width - 200, 150, 200, 20),
            elements.CylinderHead.activeSelf, "Cylinder head"));
        elements.CylinderHeadCovers.SetActive(GUI.Toggle(new Rect(Screen.width - 200, 170, 200, 20),
            elements.CylinderHeadCovers.activeSelf, "Cylinder head covers"));
        elements.Gearbox.SetActive(GUI.Toggle(new Rect(Screen.width - 200, 190, 200, 20), elements.Gearbox.activeSelf,
            "Gearbox"));
        elements.ExhaustManifolds.SetActive(GUI.Toggle(new Rect(Screen.width - 200, 210, 200, 20),
            elements.ExhaustManifolds.activeSelf, "Exhaust manifold"));
        elements.FuelRail.SetActive(GUI.Toggle(new Rect(Screen.width - 200, 230, 200, 20), elements.FuelRail.activeSelf,
            "Fuel rail"));
        elements.IntakeManifolds.SetActive(GUI.Toggle(new Rect(Screen.width - 200, 250, 200, 20),
            elements.IntakeManifolds.activeSelf, "Intake manifold"));
        elements.Flywheel.SetActive(GUI.Toggle(new Rect(Screen.width - 200, 270, 200, 20), elements.Flywheel.activeSelf,
            "Flywheel"));
        elements.Clutch.SetActive(GUI.Toggle(new Rect(Screen.width - 200, 290, 200, 20), elements.Clutch.activeSelf,
            "Clutch"));
        elements.OilPan.SetActive(GUI.Toggle(new Rect(Screen.width - 200, 310, 200, 20), elements.OilPan.activeSelf,
            "Oil pan"));
        elements.SparkPlugWires.SetActive(GUI.Toggle(new Rect(Screen.width - 200, 330, 200, 20),
            elements.SparkPlugWires.activeSelf, "Spark plug wires"));
        elements.SparkPlugs.SetActive(GUI.Toggle(new Rect(Screen.width - 200, 350, 200, 20),
            elements.SparkPlugs.activeSelf, "Spark plugs"));
    }

    public void ObjectToggle(GameObject obj)
    {
        obj.SetActive(!obj.activeInHierarchy);
    }

    private int variationG = 0;

    public void SetVariation(int variation)
    {
        if (variationG == variation)
        {
            return;
        }

        if (!allTogglesType[variation].isOn)
        {
            return;
        }

        variationG = variation;

        foreach (Toggle togg in allToggles)
        {
            togg.isOn = true;
        }
        ...
        foreach (EngineVariation _variation in engineVariations)
        {
            foreach (GameObject gameobject in _variation.gameObjects)
            {
                gameobject.SetActive(false);
            }
        }

        foreach (GameObject gameobject in engineVariations[variation].gameObjects)
        {
    ...
    {
        foreach (MeshRenderer mr in transform.GetComponentsInChildren<MeshRenderer>(true))
        {
    ...
    public void EnableTransparency()
    {
        ...
        foreach (GameObject go in TransparentGameobjects)
        {
    ...
    private IEnumerator EnableTransparencyCor(GameObject go)
    {
        ...
        fadeMaterial.SetTexture("_BaseMap", thisMaterial.GetTexture("_BaseMap"));
    ...
    private IEnumerator DisableTransparencyCor(GameObject go)
    {
        ...
        opaqMaterial.SetTexture("_BaseMap", tempMaterial.GetTexture("_BaseMap"));
    ...
    private void Update()
    {
        ...
        //    new Vector3(ZoomSlider.value, ZoomSlider.value, ZoomSlider.value);
